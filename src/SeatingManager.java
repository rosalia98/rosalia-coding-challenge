...
public class SeatingManager {
    ...
    private Map<Integer, Stack<Table>> tablesMap = new HashMap<>();
    ...
    public void arrives(CustomerGroup group){
        ...
        if(tablesMap.containsKey(groupSize)) {
            ...
            if(nrOfEmptySeats != 0) {
                if(tablesMap.get(nrOfEmptySeats).size() > 0) {
                    tablesMap.get(nrOfEmptySeats).add(tableOccupied);
                } else { // there's no other table at the moment that has the same nr of seats empty => new entry in the map
